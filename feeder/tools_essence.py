...
def random_choose(data_numpy, size, label, auto_pad=True):
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
    if T == size:
        return data_numpy
        ...
        if label != 0:
            set_begin = False
            for m in range(M):
                all_zeros = not data_numpy[:, :, :, m].any()
                if not all_zeros:
                    # print("all_zeros")
                    # print(data_numpy)
                    status = 0
                    ...
                    max_index = 0
                    for t in range(T):
                        frame_all_zeros = not data_numpy[2, t, :, m].any()
                        if status == 0:
                            if not frame_all_zeros:
                                min_index = t
                                status = 1
                        if status == 1:
                            if frame_all_zeros:
                    ...
                    if max_index - size < 0:
                        ...
                        start_index = max_index - size
                    if min_index > T:
                        end_index = T
                    else:
                        end_index = min_index
                    begin = random.randint(start_index, end_index)
                    set_begin = True
            if not set_begin:
