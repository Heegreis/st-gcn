...
from net.utils.tgcn_AplusTrain import ConvTemporalGraphical
...
class Model(nn.Module):
    ...
    def __init__(self, in_channels, num_class, graph_args,
        ...
        Aplus_Gaussian = torch.normal(mean=mean, std=std)
        self.Aplus = nn.Parameter(Aplus_Gaussian.unsqueeze(0))
    ...
    def forward(self, x, writer=None, current_iter=None):
        ...
        for gcn, importance in zip(self.st_gcn_networks, self.edge_importance):
            x, _ = gcn(x, self.A * importance, self.Aplus)
...
class st_gcn(nn.Module):
    ...
    def forward(self, x, A, Aplus):

        res = self.residual(x)
        x, A = self.gcn(x, A, Aplus)
        x = self.tcn(x) + res
